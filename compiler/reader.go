...
func (self *reader) readProto(parentSource string) *FunctionProto {
	...
	return &FunctionProto{
		...
		DbgSourcePositions:	self.readLineInfo(),
		DbgLocVars:         self.readLocVars(),
		DbgUpvalues:    	self.readUpvalueNames(),
	}
}
...
func (self *reader) readProtos(parentSource string) []*FunctionProto {
	protos := make([]*FunctionProto, self.readUint32())
	for i := range protos {
...
func (self *reader) readLocVars() []DbgLocVar {
	locVars := make([]DbgLocVar, self.readUint32())
	for i := range locVars {
		locVars[i] = DbgLocVar{
