...
func OpenUTF8Lib(ls *LuaState) int {
	ls.NewLib(utf8Lib)
	ls.Push(LuaString(UTF8PATT))
	luaSetField(ls, -2, "charpattern")
	return 1
}
...
func utfByteOffset(ls *LuaState) int {
	...
	i = posRelat(luaOptInteger(ls, 3, int64(i)), sLen)
...
func utfChar(ls *LuaState) int {
	...
	ls.Push(LuaString(_encodeUtf8(codePoints)))
	return 1
}
